...
#for (input_root in maps){
  ...
  output_file=sprintf("%s_individual_perf.csv",input_root)
  
  ref.ppp<-ppp_read(input_ref)
  data.ppp <- ppp_read(input_volunteer,min=0)
  print(unique(data.ppp$marks))
  ...
  groups=generate_combinaisons(n=ncol(data), k=1,200)    
...
benchmarks$fp_errors=(1-benchmarks$specificity)*(1-density)
benchmarks$fn_errors=(1-benchmarks$sensitivity)*(density)
benchmarks$pr_errors=1-(benchmarks$sensitivity*density+benchmarks$specificity*(1-density))

write.csv(benchmarks,output_file)
